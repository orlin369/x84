...
def banner():
    ...
    from bbs import getterminal, showart
    import os
    term = getterminal()
    banner = '\r\n'
    for line in showart(
...
def chat(sessions):
    ...
    """
    from bbs import gosub, getsession
...
def view(sessions):
    ...
    """
    from bbs import echo, getterminal
    term = getterminal()
    (node, tgt_session) = get_node(sessions)
    if node is not None:
